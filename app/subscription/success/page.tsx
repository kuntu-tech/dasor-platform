...
export default function SubscriptionSuccessPage() {
  ...
  const handleSyncStatus = async (vendorId: string, sessionIdValue?: string | null) => {
    ...
    try {
      ...
      setSyncError(error instanceof Error ? error.message : "Failed to sync subscription status")
        ...
        <CardContent className="space-y-4">
          ...
          {syncError && (
            <div className="rounded-md bg-orange-50 dark:bg-orange-900/20 p-3 text-sm text-orange-700 dark:text-orange-400">
              ⚠ {syncError} (Data may have been saved if Webhook has already processed it)
            </div>
          )}
          ...
          <div className="text-center text-sm text-muted-foreground mt-4">
            <p>Thank you for your subscription! You can now use all Pro features.</p>
          </div>
        </CardContent>
      </Card>
    </div>
