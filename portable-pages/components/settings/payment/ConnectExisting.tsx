...
const ConnectExisting = ({ onBack, onConnect }: ConnectExistingProps) => {
  ...
  // 灰色按钮：用已有Stripe Account ID绑定，始终传真实邮箱和users.id
  ...
  return (
    <div>
      ...
      <div className="max-w-2xl rounded-xl border border-border bg-card p-8">
        <Button onClick={handleStartOnboarding} className="w-full gap-2 bg-black text-white" size="lg" disabled={loading}>
          Connect with Stripe
        ...
        <div className="space-y-3">
          ...
          <Button onClick={handleBindWithAccountId} className="w-full bg-gray-500 text-white" size="lg" disabled={!accountId.trim() || loading}>
